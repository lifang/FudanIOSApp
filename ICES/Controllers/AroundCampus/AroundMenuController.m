...
@interface AroundMenuController ()<UITableViewDataSource,UITableViewDelegate,RefreshDelegate,UIAlertViewDelegate>
...
@implementation AroundMenuController
...
- (void)initAndLayoutUI {
    ...
    AroundHeaderView *headerView = [[AroundHeaderView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 256)
                                                                    target:self
                                                                    action:@selector(aroundMenuSelected:)];
...
- (IBAction)aroundMenuSelected:(id)sender {
    NSInteger tag = -1;
    if ([sender isKindOfClass:[UIButton class]]) {
        tag = [(UIButton *)sender tag];
    }
    else if ([sender isKindOfClass:[UITapGestureRecognizer class]]) {
        tag = [(AroundMenuView *)[(UITapGestureRecognizer *)sender view] index];
    }
    if (tag <= AroundMenuOther) {
        AroundListController *aroundC = [[AroundListController alloc] init];
        aroundC.aroundType = (AroundMenuType)tag;
        [self.navigationController pushViewController:aroundC animated:YES];
    }
    else {
        AroundMapListController *mapListC = [[AroundMapListController alloc] init];
        mapListC.aroundType = (AroundMenuType)tag;
        [self.navigationController pushViewController:mapListC animated:YES];
    }
}
...
- (void)downloadDataWithPage:(int)page isMore:(BOOL)isMore {
    ...
    [NetworkInterface aroundListWithStudentID:delegate.userID token:delegate.token type:AroundMenuAll page:page rows:kPageSize finished:^(BOOL success, NSData *response) {
        if (success) {
